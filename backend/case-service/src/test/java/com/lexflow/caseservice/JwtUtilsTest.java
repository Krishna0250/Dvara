...
public class JwtUtilsTest {
    ...
    void testGenerateAndValidateToken_Success() {
        String token = jwtUtils.generateToken("usr-123", "judge@lexflow.gov", "JUDGE");
        ...
        assertEquals("judge@lexflow.gov", jwtUtils.getEmailFromToken(token));
    }

    @Test
    @DisplayName("Should reject tampered or corrupted JWT tokens")
    void testValidateToken_TamperedToken_ReturnsFalse() {
        String token = jwtUtils.generateToken("usr-456", "advocate@lexflow.org", "ADVOCATE");
