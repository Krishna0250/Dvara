...
import type { NextActionEngineResult } from '../../types/legal';
...
export const NextActionCard: React.FC<NextActionCardProps> = ({ 
  ...
  return (
    <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-xl p-5 shadow-lg border border-indigo-900/60 relative overflow-hidden">
        ...
        <div className="space-y-2 flex-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {nextAction.currentState && (
              <span className="text-[11px] font-bold bg-teal-500/20 text-teal-300 border border-teal-500/40 px-2 py-0.5 rounded">
          ...
          {/* Conditions Checked */}
          {!compact && nextAction.conditions && (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="text-xs text-slate-400 font-medium">Verified Conditions:</span>
              {nextAction.conditions.map((cond, idx) => (
                <span key={idx} className="inline-flex items-center gap-1 text-xs bg-slate-800/80 text-slate-300 px-2 py-0.5 rounded border border-slate-700">
