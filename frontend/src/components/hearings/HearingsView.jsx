...
export const HearingsView = ({ cases, onSelectCase }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const allHearings = cases.flatMap((c) => c.hearings);
  const filteredHearings = allHearings.filter(
    (h) => h.caseNumber.toLowerCase().includes(searchTerm.toLowerCase()) || h.caseTitle.toLowerCase().includes(searchTerm.toLowerCase()) || h.purpose.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredHearings.map((h) => <div key={h.id} className="legal-card p-6 space-y-4 hover:border-indigo-300 transition-all">
            ...
            <div className="space-y-2 text-xs">
              ...
              {h.previousOrderSnippet && <div className="p-2.5 bg-indigo-50/60 border border-indigo-100 rounded text-[11px] text-indigo-900">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {h.requiredDocuments.map((doc, idx) => <span key={idx} className="text-[10px] font-semibold bg-slate-100 text-slate-700 px-2 py-0.5 rounded border border-slate-200">
                      📄 {doc}
                    </span>)}
                </div>
              </div>
