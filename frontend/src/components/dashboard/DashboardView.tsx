...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const activeCasesCount = cases.filter(c => c.status === 'Active').length + 40; // Total 47 active cases
  const hearingsThisWeek = 12;
  const deadlinesCount = 7;
  const actionRequiredCount = 9;


  // Extract upcoming hearings across cases
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="legal-card p-6 space-y-4">
          ...
          <div className="space-y-3">
            {allHearings.map(h => (
              <div key={h.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200/80 flex items-center justify-between hover:bg-slate-100/80 transition-colors">
        ...
        <div className="legal-card p-6 space-y-4">
          ...
          <div className="space-y-3">
            {allDeadlines.map(d => (
              <div key={d.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200/80 flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded ${
                      d.status === 'Overdue' ? 'bg-red-100 text-red-800 border border-red-200' : 'bg-amber-100 text-amber-800 border border-amber-200'
                    }`}>
