...
export const DeadlinesView = ({ cases, onSelectCase }) => {
  ...
  return <div className="p-8 space-y-6 max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {["All", "Overdue", "Due Soon", "Upcoming"].map((st) => <button
    key={st}
    onClick={() => setFilterStatus(st)}
    className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${filterStatus === st ? "bg-slate-900 text-white shadow-xs" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}
  >
              {st}
            </button>)}
        </div>
    ...
    return <div
      key={d.id}
      className={`legal-card p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 ${isOverdue ? "border-l-red-600 bg-red-50/20" : "border-l-amber-500 bg-white"}`}
    >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${isOverdue ? "bg-red-100 text-red-800 border border-red-200" : "bg-amber-100 text-amber-800 border border-amber-200"}`}>
