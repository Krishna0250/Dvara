...
import { ShieldAlert, CheckCircle2, AlertTriangle, FileText, Send, UserCheck, ArrowRight } from "lucide-react";
import { api } from "../../api/apiClient";
export const ScrutinyView = ({ cases, currentRole, onRefreshCases, onSelectCase }) => {
  ...
  const loadDeficiencies = async (caseId) => {
    setLoading(true);
  ...
  const handleResolveDeficiency = async (defId) => {
  ...
  return <div className="p-6 max-w-7xl mx-auto space-y-6">
    ...
    return <div
      ...
      className={`p-3.5 rounded-xl cursor-pointer border transition-all ${isSelected ? "border-indigo-600 bg-indigo-50/60 shadow-xs" : "border-slate-200 hover:border-slate-300 bg-white"}`}
    >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="text-xs font-bold text-indigo-700">{c.filingId || c.caseNumber}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${status === "DEFICIENT" ? "bg-rose-100 text-rose-800 border border-rose-300" : status === "REGISTERED" ? "bg-emerald-100 text-emerald-800 border border-emerald-300" : "bg-amber-100 text-amber-800 border border-amber-300"}`}>
                      {status}
                    </span>
                  </div>
    ...
    /* Right: Detailed Scrutiny & Deficiency Workspace */
        ...
        {selectedCase && <div className="lg:col-span-8 space-y-6">
            {
    /* Case Header Card */
  }
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {selectedCase.caseNumber && <span className="text-xs font-black text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-200">
  ...
  >
    ...
    /* Status Actions */
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                ...
                <div className="flex items-center gap-2">
                  {(currentRole === "SCRUTINY_OFFICER" || currentRole === "ADMIN") && <button
  ...
  >
                  ...
                  {(currentRole === "REGISTRAR" || currentRole === "ADMIN") && selectedCase.filingStatus !== "REGISTERED" && <button
  ...
  >
    ...
    /* Registration Config Modal / Controls if Registrar */
              ...
              {(currentRole === "REGISTRAR" || currentRole === "ADMIN") && selectedCase.filingStatus !== "REGISTERED" && <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-3">
  ...
  />
    ...
    /* Deficiencies List Section */
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 space-y-4">
              ...
              {loading ? <div className="p-8 text-center text-xs text-slate-400">Loading audit deficiencies...</div> : deficiencies.length === 0 ? <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200 text-xs text-slate-500">
                  <CheckCircle2 className="w-6 h-6 text-emerald-500 mx-auto mb-2" />
                  No open procedural deficiencies found for this petition. Ready for registration approval.
                </div> : <div className="space-y-3">
                  {deficiencies.map((def) => <div
    key={def.id}
    className={`p-4 rounded-xl border transition-all ${def.status === "OPEN" ? "border-rose-200 bg-rose-50/40" : "border-emerald-200 bg-emerald-50/40"}`}
  >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-slate-900">{def.documentTitle || "Filing Document"}</span>
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${def.status === "OPEN" ? "bg-rose-100 text-rose-800" : "bg-emerald-100 text-emerald-800"}`}>
                              {def.status}
                            </span>
                          </div>
    ...
    /* Remark / Resolution Box */
                      ...
                      {def.remark && <div className="mt-3 p-2.5 bg-white/80 border border-slate-200 rounded-lg text-xs text-slate-700">
    ...
    /* Resolve Controls for Advocate/Citizen */
                      ...
                      {def.status === "OPEN" && (currentRole === "ADVOCATE" || currentRole === "CITIZEN" || currentRole === "ADMIN") && <div className="mt-3 pt-3 border-t border-rose-200 space-y-2">
                          {activeDefId === def.id ? <div className="space-y-2">
                              <textarea
    value={resolveRemark}
  ...
  >
    ...
    /* Modal: Raise Deficiency */
      ...
      {showRaiseModal && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
