...
export const CreateCaseWizard = ({
  ...
  const [applicableLaw, setApplicableLaw] = useState("Section 302 IPC / Section 103 BNSS");
  const [court, setCourt] = useState("Sessions Court, Division I");
  const [jurisdiction] = useState("District Court Jurisdiction");
  const [caseNumber, setCaseNumber] = useState(`CR-2026-${Math.floor(100 + Math.random() * 900)}`);
  const [title, setTitle] = useState("State vs Vivek Kumar");
  const [filingDate, setFilingDate] = useState((/* @__PURE__ */ new Date()).toISOString().split("T")[0]);
  const [priority, setPriority] = useState("High");
  const [assignedLawyer, setAssignedLawyer] = useState("Adv. Rajesh Verma");
  const [summary, setSummary] = useState("Criminal trial proceedings initialized under statutory framework.");
  const [complainantName, setComplainantName] = useState("State of Maharashtra");
  const [respondentName, setRespondentName] = useState("Vivek Kumar");
  const [nextHearingDate, setNextHearingDate] = useState("2026-09-25");
  const [nextHearingPurpose, setNextHearingPurpose] = useState("Pre-Trial Scrutiny & Summons Verification");
  ...
  return <div className="p-8 max-w-4xl mx-auto space-y-6">
      {
    /* Wizard Header */
  }
  ...
  >
    ...
    /* Multi-Step Indicator Bar */
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        {steps.map((s) => <div key={s.num} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${currentStep === s.num ? "bg-indigo-600 text-white ring-4 ring-indigo-100" : currentStep > s.num ? "bg-teal-600 text-white" : "bg-slate-200 text-slate-500"}`}>
  ...
  >
                ...
                {LAW_PROVISIONS.map((p) => <option key={p.id} value={p.id}>
  ...
  />
    ...
    /* Step 6: Workflow Preview */
        ...
        {currentStep === 6 && <div className="space-y-5">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
              {activeTemplate.stages.map((stg, idx) => <div key={stg.id} className="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between text-xs">
                  ...
                  {stg.isConditional && <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-2 py-0.5 rounded flex items-center gap-1">
