...
export const CaseDetailView = ({
  ...
  const [orderType, setOrderType] = useState("PROCEDURAL");
  ...
  >
    ...
    return <button
      ...
      className={`pb-3 flex items-center gap-2 border-b-2 transition-all cursor-pointer ${isActive ? "border-indigo-600 text-indigo-600 font-extrabold" : "border-transparent text-slate-500 hover:text-slate-800"}`}
    >
  ...
  >
              ...
              <div className="space-y-3">
                {caseData.timeline.slice(-3).reverse().map((evt) => <div key={evt.id} className="p-3 bg-slate-50 rounded-lg border border-slate-200/80 flex items-start gap-3 text-xs">
    ...
    /* Right Column: Key Details Sidebar */
          ...
          <div className="space-y-6">
            <div className="legal-card p-6 space-y-4">
              ...
              {caseData.hearings.length > 0 ? <div className="space-y-3 text-xs">
                  ...
                  <div>
                    ...
                    <ul className="space-y-1 pl-4 list-disc text-slate-600">
                      {caseData.hearings[0].requiredDocuments.map((doc, idx) => <li key={idx}>{doc}</li>)}
                    </ul>
                  </div>
                </div> : <p className="text-xs text-slate-500">No hearings currently scheduled.</p>}
            </div>
    ...
    /* Parties Summary */
            ...
            <div className="legal-card p-6 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                {caseData.parties.map((p) => <div key={p.id} className="p-2 rounded bg-slate-50 flex items-center justify-between">
                    <div>
    ...
    /* Timeline Tab */
      ...
      {activeTab === "timeline" && <div className="legal-card p-6 space-y-6">
  ...
  >
          ...
          <div className="relative pl-6 space-y-6 border-l-2 border-slate-200">
            {caseData.timeline.map((evt) => <div key={evt.id} className="relative group">
                <div className="absolute -left-[31px] top-0 w-4 h-4 rounded-full bg-indigo-600 border-2 border-white ring-2 ring-indigo-100" />
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-2 border-t border-slate-200/60">
                    ...
                    {evt.documentName && <span className="text-indigo-600 font-semibold flex items-center gap-1">
                        📄 {evt.documentName}
                      </span>}
                  </div>
                </div>
  ...
  />}
    ...
    /* Documents Tab */
      ...
      {activeTab === "documents" && <div className="legal-card p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {caseData.documents.map((doc) => <div key={doc.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex items-start justify-between">
    ...
    /* Hearings Tab */
      ...
      {activeTab === "hearings" && <div className="legal-card p-6 space-y-4">
          <h2 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-3">Scheduled Hearings & Prep</h2>
          <div className="space-y-3">
            {caseData.hearings.map((h) => <div key={h.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <div className="flex items-center justify-between">
    ...
    /* Judicial Orders Tab */
      ...
      {activeTab === "orders" && <div className="legal-card p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
            ...
            {(currentRole === "JUDGE" || currentRole === "REGISTRAR" || currentRole === "ADMIN") && <button
  ...
  >
          ...
          {orders.length === 0 ? <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed text-xs text-slate-500">
              No formal judicial orders issued yet for this case.
            </div> : <div className="space-y-4">
              {orders.map((ord) => <div key={ord.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] text-slate-500 flex items-center justify-between">
                    ...
                    {ord.documentUrl && <a href="#" className="text-indigo-600 font-bold hover:underline">Download Signed Order PDF</a>}
                  </p>
                </div>)}
            </div>}
        </div>}
    ...
    /* Audit Log Tab */
      ...
      {activeTab === "audit" && <div className="legal-card p-6 space-y-4">
          ...
          {auditLogs.length === 0 ? <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed text-xs text-slate-500">
              No backend audit logs recorded yet.
            </div> : <div className="space-y-3">
              {auditLogs.map((log) => <div key={log.id} className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex items-start justify-between text-xs">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {log.previousState && log.newState && <span className="text-[10px] text-slate-500 font-mono">
    ...
    /* Modal: Issue Judicial Order */
      ...
      {showOrderModal && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
  ...
  >
    ...
    /* Log Event Modal */
      ...
      {showAddEventModal && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
