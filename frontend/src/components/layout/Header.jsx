...
import { Search, Bell, Plus, PlayCircle, ChevronDown } from "lucide-react";
export const Header = ({ onOpenCreateCase, onSelectCase, cases, currentRole, onRoleChange }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [showDemoMenu, setShowDemoMenu] = useState(false);
  const [showRoleMenu, setShowRoleMenu] = useState(false);
  const filteredCases = cases.filter(
    (c) => c.caseNumber.toLowerCase().includes(searchQuery.toLowerCase()) || c.title.toLowerCase().includes(searchQuery.toLowerCase()) || c.caseType.toLowerCase().includes(searchQuery.toLowerCase())
  );
  return <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-10 shadow-xs">
      {
    /* Search Bar */
  }
  ...
  />
        {searchQuery && <div className="absolute left-0 right-0 top-11 bg-white border border-slate-200 rounded-lg shadow-xl py-1 z-30 max-h-64 overflow-y-auto">
            {filteredCases.length > 0 ? filteredCases.map((c) => <button
    key={c.id}
    onClick={() => {
      onSelectCase(c.id);
      setSearchQuery("");
  ...
  >
                ...
                </button>) : <div className="px-4 py-3 text-xs text-slate-500">No matching cases found</div>}
          </div>}
      </div>
  ...
  >
          ...
          {showDemoMenu && <div className="absolute right-0 top-10 w-72 bg-white border border-slate-200 rounded-xl shadow-xl p-2 z-30 space-y-1">
  ...
  >
          ...
          {showRoleMenu && <div className="absolute right-0 top-10 w-64 bg-white border border-slate-200 rounded-xl shadow-xl p-2 z-30 space-y-1">
              <div className="px-2 py-1 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                Switch Active Persona / RBAC
              </div>
              {[
    { role: "JUDGE", name: "Hon'ble Justice Sikri", badge: "Judge / Bench" },
    { role: "REGISTRAR", name: "Registrar Deshmukh", badge: "Court Admin" },
    { role: "SCRUTINY_OFFICER", name: "Officer Priya Nair", badge: "Filing Audit" },
    { role: "ADVOCATE", name: "Adv. Rajesh Verma", badge: "Legal Counsel" },
    { role: "CITIZEN", name: "Rohan Kumar", badge: "Litigant" }
  ].map((r) => <button
    key={r.role}
    onClick={() => {
      onRoleChange(r.role);
    ...
    className={`w-full text-left p-2 rounded-lg transition-colors flex items-center justify-between group ${currentRole === r.role ? "bg-indigo-600 text-white" : "hover:bg-slate-100 text-slate-900"}`}
  >
                  <div>
                    <div className="text-xs font-bold">{r.name}</div>
                    <div className={`text-[10px] ${currentRole === r.role ? "text-indigo-100" : "text-slate-500"}`}>{r.badge}</div>
                  </div>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${currentRole === r.role ? "bg-white/20 text-white" : "bg-slate-200 text-slate-700"}`}>
                    {r.role}
                  </span>
                </button>)}
            </div>}
